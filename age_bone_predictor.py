...
def main():
    ...
    for modelo_version in modelo_version_list:
        ...
        if os.path.exists(model_weights_path):
            ...
            try:
                modelo_entrenado.load_state_dict(
                    torch.load(model_weights_path, map_location=device)
                )
            except RuntimeError as e:
                log.error(f"Error al cargar los pesos: {e}")

            print(
                f"Valores del entrenamiento del rendimiento del modelo: {model_weights_path}"
            )
            ...
            for id_mg in ids_imgs:
                ...
                suma += error
                log.debug(f"Predicción de edad ósea para la imagen {df_train['id'].iloc[id_mg]}: {prediccion:.2f}")
            ...
            log.info(
                "Iniciando fine-tuning del modelo ViT_B_16 para predecir boneage..."
            )
            ...
            for alpha in alphas:
                ...
                modelo_entrenado = fine_tuning_model(
                    df_train, img_train_dataset, num_epochs=10, batch_size=16, lr=alpha
                )
