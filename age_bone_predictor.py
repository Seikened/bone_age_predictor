...
def main():
    ...
    if os.path.exists(model_weights_path):
        ...
        modelo_entrenado = fine_tuning_model(
            df_train, img_train_dataset, num_epochs=10, batch_size=16, lr=1e-4
        )
    ...
    print(
        f"Predicción de edad ósea para la imagen {df_train['id'].iloc[id_mg]}: {prediccion:.2f}"
    )

    print(
        f"Valor real: {df_train['boneage'].iloc[id_mg]} | Predicción: {prediccion:.2f} | Error: {abs(df_train['boneage'].iloc[id_mg] - prediccion):.2f} "
    )
